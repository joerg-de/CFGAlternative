...
{
    std::vector<bool> washerefild(nodes.size(),false);
    std::vector<bool> washerefild2(nodes.size(),false);
    entry.setDepth(0); //the entry is normaly the one at the top
    step1(entry,washerefild,washerefild2);
    ...
    do
    {
        ...
        {
            if (washerefild[node->getID()])
            {
                for(auto l1 : *node->getOutEdges())
                {
                    auto t1 = l1->getDest();
                    if (!washerefild[t1->getID()])
                    {
        ...
        if(foundsrc)
        {
            ...
            washerefild[foundtar->getID()] = true;

            step1(*foundtar,washerefild,washerefild2);
            donesomething = true;
        }
    }
    while (donesomething);
...
void CFG::step1(CFGNode &entry, std::vector<bool> &washerefild, std::vector<bool> &washerefild2)
{
    //init the first
    washerefild[entry.getID()] = true;
    ...
    while (!mystack.empty())
    {
        ...
        mystack.pop_front();
        washerefild2[current->getID()] = true;
        //place all the matching
        for(CFGEdge* l1 : *current->getOutEdges())
        {
            auto dest_node = l1->getDest();
            //TODO the last one possible should place this --> that excludes all that are donated by the block itself + all that go trough an obj that is already placed
            bool cont = false;
            ...
            if(size != 1)
            {
                ...
                for(CFGEdge* l2 : *dest_node->getInEdges())
                {
                    ...
                    if(!washerefild2[src->getID()])
                    {
                        int res = search.isconnected(entry.getID(),src->getID());
                        if(res != 0)
                        {
                            cont = true;
                            break;
            ...
            if(!washerefild[dest_node->getID()])
            {
                current->hasplaced(*dest_node);

                dest_node->setDepth(current->getDepth()  + 1);
                washerefild[dest_node->getID()] = true;
