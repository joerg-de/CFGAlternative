...
{
    int debugtempval = 0;
    //buble sort because it needs to end at some point todo more effective sorting todo only sort each Linebinder once
    for(unsigned int i = 0;i != ownedVerticalLines.size();++i)
    {
        for(unsigned int j = 0;j != ownedVerticalLines.size() - 1;++j)
        {
            ...
            if(pull < 0)
            {
                ...
                if(diff == 0)
                {
                    ...
                    {
                        LineBinder::swapLineHolder(ownedVerticalLines[j]->getUsedLineBinder(),ownedVerticalLines[next]->getUsedLineBinder());
                    }
                }
